...
一款功能强大的应用层反汇编代码注入器，该工具可用于向第三方进程内强制插入`DLL`模块，也可用于插入一段`ShellCode`反弹后门，或实现汇编级`Call`调用功能，在后渗透中，也可将后门直接注入到特定进程内存内而不会在磁盘中留下任何痕迹，注入成功后`Metasploit`即可获取目标主机控制权，只要对端不关机则权限会一直维持，由于`内存注入`无对应磁盘文件，所以也不会触发杀软报毒，是一款不错的命令行版通用注入工具。
